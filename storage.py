...
CTA_PATTERNS = [
    'discover more', 'scopri di più', 'mehr erfahren', 'découvrir', 'descubre más', 'ontdek meer', 'daha fazla', 'learn more'
]
...
def _count_cta(text: str) -> tuple[str, int] | None:
    lower = text.lower()
    for cta in CTA_PATTERNS:
        count = lower.count(cta)
        if count >= 4:
            return cta, count
    return None
    ...
    for page in pages:
        if page.status_code is None or page.status_code >= 400:
            ...
            findings.append(_finding(
                page, 'Codice/Tecnica', 'Alta',
                'Pagina linkata ma non accessibile' if confidence == 'Alta' else 'Pagina seed non accessibile',
                'Una pagina del sito restituisce un errore HTTP oppure non è stata caricata correttamente durante il crawl.',
                'L’utente può incontrare una pagina non disponibile, interrompere il percorso oppure percepire bassa affidabilità del sito.',
                'Verificare status code, redirect, URL reali presenti nel menu/homepage e possibili blocchi bot o firewall.',
                f'Status code: {page.status_code}; URL: {page.final_url or page.url}; scoperta da: {page.discovered_from or "n/d"}',
                confidence,
            ))
        ...
        if re.search(r'\bundefined\b', page.text, re.IGNORECASE) or 'undefined' in (page.final_url or '').lower():
            findings.append(_finding(
                page, 'Codice/Tecnica', 'Alta', 'Placeholder tecnico visibile',
        ...
        if page.language in NON_ENGLISH and re.search(r'where are you\?', page.text, re.IGNORECASE):
            findings.append(_finding(
                page, 'Contenuti/Localizzazione', 'Media', 'Placeholder dealer non localizzato',
        ...
        if re.search(r'<h[1-6][^>]*>\s*</h[1-6]>', page.html, re.IGNORECASE):
            findings.append(_finding(
                page, 'Accessibilità', 'Media', 'Heading vuoto nel markup',
        ...
        if stale_years and page.page_type == 'home':
            findings.append(_finding(
                page, 'Contenuti/Localizzazione', 'Media', 'Anno passato visibile in homepage',
        ...
        for marker in markers:
            if marker.lower() in page.text.lower():
                findings.append(_finding(
                    page, 'Contenuti/Localizzazione', 'Media', 'Possibile leakage di altra lingua',
        ...
        if broken_pattern:
            findings.append(_finding(
                page, 'Codice/Tecnica', 'Critica', 'URL malformato nei link interni',
        ...
        if page.errors:
            findings.append(_finding(
                page, 'Codice/Tecnica', 'Bassa', 'Errori durante il crawling',
        ...
        cta_count = _count_cta(page.text)
        if cta_count and page.page_type == 'home':
            cta, count = cta_count
            findings.append(_finding(
                page, 'UX/UI', 'Bassa', 'CTA molto ripetute in homepage',
                'La homepage ripete molte volte la stessa call to action, riducendo chiarezza e priorità del percorso.',
                'L’utente può percepire una homepage rumorosa e meno orientata al task principale.',
                'Differenziare le CTA per obiettivo e ridurre i blocchi ridondanti.',
                f'CTA rilevata: {cta}; occorrenze circa: {count}', 'Media'
            ))

        if page.page_type in {'home', 'catalogue'} and ('dealer' not in page.text.lower()) and ('rivenditor' not in page.text.lower()):
            findings.append(_finding(
                page, 'CRO', 'Bassa', 'Dealer journey poco visibile nella pagina',
                'Nella pagina non emerge in modo chiaro un accesso al percorso dealer o al punto vendita.',
                'Può ridurre la capacità del sito di portare l’utente verso il contatto o il rivenditore.',
                'Valutare maggiore evidenza del dealer finder nelle pagine ad alto traffico.',
                'Nessun marker dealer rilevato nel testo della pagina.', 'Bassa'
            ))
